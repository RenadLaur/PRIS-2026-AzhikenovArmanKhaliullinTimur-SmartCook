...
## NLP (Lab 6): NER + Lemmatization

В проект добавлен NLP-модуль для извлечения сущностей из текста:
- `Имена` (PER/PERSON)
- `Даты` (DATE)
- `Локации` (LOC/GPE)
- `Организации` (ORG)
- Лемматизация (топ лемм)

Поддерживаются сценарии разбора:
- резюме
- отзыв
- жалоба
- свободный текст
...
Для spaCy (опционально, если хотите использовать spaCy-модель):
...
- Отправьте сообщение вида:  
  `/nlp Меня зовут Айдана Нургалиева. Я работала в Алматы с 12.09.2022.`
- Или используйте блок `NLP Analyzer (NER + Lemmatization)` на странице.
